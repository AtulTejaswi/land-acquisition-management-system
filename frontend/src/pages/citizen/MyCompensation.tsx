  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-slate-900">{t('citizen.myCompensation.title')}</h1>
        <p className="text-slate-500 text-sm">{t('citizen.myCompensation.subtitle')}</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 border-emerald-200">
          <CardContent className="p-4">
            <div className="text-sm text-emerald-600 font-medium">{t('citizen.myCompensation.totalAwarded')}</div>
            <div className="text-2xl font-bold text-emerald-700 tabular-nums">{formatCurrency(totalAwarded)}</div>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100/50 border-blue-200">
          <CardContent className="p-4">
            <div className="text-sm text-blue-600 font-medium">{t('citizen.myCompensation.totalDisbursed')}</div>
            <div className="text-2xl font-bold text-blue-700 tabular-nums">{formatCurrency(totalPaid)}</div>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-br from-amber-50 to-amber-100/50 border-amber-200">
          <CardContent className="p-4">
            <div className="text-sm text-amber-600 font-medium">{t('citizen.myCompensation.pending')}</div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('citizen.myCompensation.compensationAwards')}</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <EmptyState icon="📋" title={t('citizen.myCompensation.noRecords')} description={t('citizen.myCompensation.noRecordsDesc')} />
          ) : (
            <div className="space-y-3">
              {compensations?.items?.map((comp: any) => (
                <div key={comp.id} className="border border-slate-200 rounded-lg p-4 hover:shadow-sm transition-shadow">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="font-semibold text-slate-900">{t('citizen.myCompensation.awardAmount')} {formatCurrency(Number(comp.total_award))}</div>
                      <div className="text-sm text-slate-500 mt-1 space-x-4">
                        ...
                        {comp.additional_compensation > 0 && (
                          <span>{t('citizen.myCompensation.additional')}: <span className="tabular-nums">{formatCurrency(Number(comp.additional_compensation))}</span></span>
                        )}
                      </div>
                      <div className="text-xs text-slate-400 mt-1">
                        {t('citizen.myCompensation.assessed')} {formatDate(comp.assessment_date)}
                      </div>
                    </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('citizen.myCompensation.paymentHistory')}</CardTitle>
        </CardHeader>
        <CardContent>
          {payments?.items?.length === 0 ? (
            <EmptyState icon="🏦" title={t('citizen.myCompensation.noPayments')} description={t('citizen.myCompensation.noPaymentsDesc')} />
          ) : (
            <div className="space-y-3">
                ...
                <div key={pay.id} className="border border-slate-200 rounded-lg p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="text-xs text-slate-500 mt-1">{t('citizen.trackStatus.pfmsRef')}: {pay.pfms_reference || '—'}</div>
                      <div className="text-xs text-slate-500">{t('citizen.trackStatus.bankVerification')}: {pay.bank_verification_status?.replace(/_/g, ' ')}</div>
                      {pay.disbursed_date && <div className="text-xs text-slate-400">{t('citizen.myCompensation.disbursed')} {formatDate(pay.disbursed_date)}</div>}
                    </div>
