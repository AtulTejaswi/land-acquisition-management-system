...
# Use a synchronous engine for the one-shot seed script
    ...
    try:
            ...
            for i, dist_name in enumerate(districts_list[:2], 1):
                ...
                for j, village_name in enumerate(villages_list[:2], 1):
                    village = Village(
                        ...
                        tehsil=f"{dist_name} Tehsil",
                        code=f"{state_code}{i:02d}{j:02d}",
                    )
                    db.add(village)
                    db.flush()
        ...
        for i, (state, count) in enumerate([(state_objects[0], 2), (state_objects[1], 2)]):
            for j in range(count):
                u = User(
                    full_name=f"State Officer {i * 2 + j + 1}",
                    email=f"state{i * 2 + j + 1}@nlams.gov.in",
                    phone=f"+91-98100-00{j + 2:02d}",
                    password_hash=password_hash,
                    role_id=roles["state_authority"].id,
                    state_id=state.id,
                    is_active=True,
                )
                db.add(u)
                db.flush()
                state_auth_users.append(u)
                all_users.append(u)
        ...
        for i, state in enumerate(state_objects[:4]):
            for j in range(2):
                ...
                if dist:
                    u = User(
                        full_name=f"District Officer {i * 2 + j + 1}",
                        email=f"district{i * 2 + j + 1}@nlams.gov.in",
                        phone=f"+91-98100-0{j + 11:02d}",
                        password_hash=password_hash,
                        role_id=roles["district_officer"].id,
                        state_id=state.id,
                        district_id=dist.id,
                        is_active=True,
                    )
        ...
        for i in range(4):
            u = User(
                full_name=f"Agency Rep {i + 1}",
                email=f"agency{i + 1}@nhai.gov.in",
                phone=f"+91-98100-000{i + 5}",
                password_hash=password_hash,
                role_id=roles["agency"].id,
                state_id=state_objects[i % len(state_objects)].id,
                agency_name=f"NHAI Division {i + 1}",
                is_active=True,
            )
            db.add(u)
            db.flush()
            agency_users.append(u)
            all_users.append(u)
        ...
        for i in range(6):
            ...
            u = User(
                full_name=f"Field Officer {i + 1}",
                email=f"field{i + 1}@nlams.gov.in",
                phone=f"+91-98100-000{i + 9}",
                password_hash=password_hash,
                role_id=roles["field_officer"].id,
                state_id=fo_state.id,
                is_active=True,
            )
            db.add(u)
            db.flush()
        ...
        for i in range(8):
            u = User(
                full_name=f"Land Owner {i + 1}",
                email=f"citizen{i + 1}@email.com",
                phone=f"+91-98100-00{i + 20:02d}",
                password_hash=password_hash,
        ...
        p1 = Project(
            name="NH-44 Widening — Nagpur to Betul",
            ministry_id=ministries[0].id,
            category_id=categories[0].id,
            implementing_agency_id=agency_users[0].id,
            state_id=state_objects[0].id,
            ...
            estimated_budget=Decimal("18500000000.00"),
            estimated_land_required_hectares=Decimal("1250.000"),
            priority="high",
            current_stage="physical_possession",
            status="completed",
        ...
        p2 = Project(
            name="Bhogapuram International Airport Land Pooling",
            ministry_id=ministries[0].id,
            category_id=categories[6].id,
            implementing_agency_id=agency_users[1].id,
            state_id=state_objects[7].id,
            district_id=district_objects.get(
                "Andhra Pradesh_Vijayawada", district_objects["Andhra Pradesh_Amaravati"]
            ).id,
            description="Land pooling for Bhogapuram International Airport development",
            estimated_budget=Decimal("52000000000.00"),
            estimated_land_required_hectares=Decimal("3500.000"),
            priority="critical",
            current_stage="compensation_assessment",
            status="approved",
            start_date=datetime(2021, 6, 1),
            target_completion_date=datetime(2026, 3, 31),
            created_by=agency_users[1].id,
        )
        db.add(p2)

        p3 = Project(
            name="North-South Freight Corridor - Phase 2",
            ministry_id=ministries[2].id,
            category_id=categories[1].id,
            implementing_agency_id=agency_users[2].id,
            state_id=state_objects[3].id,
            ...
            estimated_budget=Decimal("38000000000.00"),
            estimated_land_required_hectares=Decimal("2100.000"),
            priority="high",
            current_stage="gis_mapping",
            status="under_review",
            start_date=datetime(2022, 1, 10),
            target_completion_date=datetime(2025, 9, 30),
            created_by=agency_users[2].id,
        )
        db.add(p3)

        p4 = Project(
            name="Solar Power Park - Jodhpur Region",
            ministry_id=ministries[3].id,
            category_id=categories[4].id,
            implementing_agency_id=agency_users[3].id,
            state_id=state_objects[2].id,
            district_id=district_objects["Rajasthan_Jodhpur"].id,
            description="Large-scale solar power generation facility",
            estimated_budget=Decimal("8500000000.00"),
            estimated_land_required_hectares=Decimal("500.000"),
            priority="medium",
            current_stage="land_requirement",
            status="submitted",
            start_date=datetime(2023, 8, 1),
            target_completion_date=datetime(2025, 3, 31),
            created_by=agency_users[3].id,
        )
        db.add(p4)

        p5 = Project(
            name="Waghur Irrigation Project - Maharashtra",
            ...
            estimated_budget=Decimal("12000000000.00"),
            estimated_land_required_hectares=Decimal("800.000"),
            priority="high",
            current_stage="payment_disbursement",
            status="delayed",
            start_date=datetime(2020, 6, 1),
            target_completion_date=datetime(2024, 6, 30),
            created_by=sa.id,
        )
        db.add(p5)

        p6 = Project(
            name="Kosi Bridge Construction",
            ministry_id=ministries[0].id,
            category_id=categories[0].id,
            implementing_agency_id=agency_users[1].id,
            state_id=state_objects[4].id,
            ...
            estimated_budget=Decimal("4500000000.00"),
            estimated_land_required_hectares=Decimal("250.000"),
            priority="medium",
            current_stage="legal_notification",
            status="under_review",
        ...
        p7 = Project(
            name="Technology Investment Region - Hyderabad",
            ministry_id=ministries[4].id,
            category_id=categories[5].id,
            implementing_agency_id=agency_users[2].id,
            state_id=state_objects[8].id,
            district_id=district_objects["Telangana_Hyderabad"].id,
            description="Tech city development with integrated infrastructure",
            estimated_budget=Decimal("25000000000.00"),
            estimated_land_required_hectares=Decimal("600.000"),
            priority="critical",
            current_stage="project_proposal",
            status="draft",
            start_date=datetime(2024, 1, 1),
            target_completion_date=datetime(2028, 12, 31),
            created_by=sa.id,
        )
        db.add(p7)

        p8 = Project(
            name="Mumbai Coastal Road - Phase 3",
            ministry_id=ministries[0].id,
            category_id=categories[0].id,
            implementing_agency_id=agency_users[0].id,
            state_id=state_objects[0].id,
            ...
            estimated_budget=Decimal("15000000000.00"),
            estimated_land_required_hectares=Decimal("450.000"),
            priority="high",
            current_stage="dpr_upload",
            status="submitted",
            start_date=datetime(2023, 2, 1),
            target_completion_date=datetime(2026, 6, 30),
            created_by=agency_users[0].id,
        )
        db.add(p8)
        ...
        p9 = Project(
            ...
            estimated_budget=Decimal("11000000000.00"),
            estimated_land_required_hectares=Decimal("350.000"),
            priority="high",
        ...
        p10 = Project(
            name="Rishikesh Valley Irrigation & Hydroelectric",
            ministry_id=ministries[1].id,
            category_id=categories[2].id,
            implementing_agency_id=agency_users[0].id,
            state_id=state_objects[4].id,
            ...
            estimated_budget=Decimal("6800000000.00"),
            estimated_land_required_hectares=Decimal("450.000"),
            priority="medium",
            current_stage="dpr_upload",
            status="submitted",
        ...
        p11 = Project(
            name="Visakhapatnam Industrial Zone Development",
            ministry_id=ministries[4].id,
            category_id=categories[3].id,
            implementing_agency_id=agency_users[1].id,
            state_id=state_objects[7].id,
            district_id=district_objects.get(
                "Andhra Pradesh_Visakhapatnam", district_objects["Andhra Pradesh_Amaravati"]
            ).id,
            description="Industrial zone development with supporting infrastructure",
            estimated_budget=Decimal("3200000000.00"),
            estimated_land_required_hectares=Decimal("750.000"),
            priority="low",
            current_stage="gis_mapping",
            status="active",
        ...
        p12 = Project(
            name="Rajasthan Renewable Energy Park",
            ministry_id=ministries[3].id,
            category_id=categories[4].id,
            implementing_agency_id=agency_users[2].id,
            state_id=state_objects[2].id,
            ...
            estimated_budget=Decimal("9500000000.00"),
            estimated_land_required_hectares=Decimal("1200.000"),
            priority="high",
            current_stage="project_completion",
            status="completed",
            start_date=datetime(2019, 1, 1),
            target_completion_date=datetime(2024, 3, 31),
            created_by=sa.id,
        )
        db.add(p12)

        p13 = Project(
            name="DMIC - Delhi Mumbai Industrial Corridor",
            ministry_id=ministries[4].id,
            category_id=categories[3].id,
            implementing_agency_id=agency_users[3].id,
            state_id=state_objects[1].id,
            district_id=district_objects["Gujarat_Ahmedabad"].id,
            description="Industrial corridor development across Delhi-Mumbai region",
            estimated_budget=Decimal("45000000000.00"),
            estimated_land_required_hectares=Decimal("2800.000"),
            priority="critical",
            current_stage="district_verification",
            status="under_review",
            start_date=datetime(2021, 8, 1),
            target_completion_date=datetime(2026, 9, 30),
            created_by=sa.id,
        )
        db.add(p13)

        p14 = Project(
            name="Ganga Expressway - Phase 1",
            ministry_id=ministries[0].id,
            category_id=categories[0].id,
            implementing_agency_id=agency_users[0].id,
            state_id=state_objects[3].id,
            ...
            estimated_budget=Decimal("22000000000.00"),
            estimated_land_required_hectares=Decimal("1800.000"),
            priority="critical",
            current_stage="physical_possession",
            status="active",
            start_date=datetime(2022, 10, 1),
            target_completion_date=datetime(2027, 3, 31),
            created_by=sa.id,
        )
        db.add(p14)

        p15 = Project(
            name="Bhubaneswar Smart City Initiative",
            ministry_id=ministries[4].id,
            category_id=categories[5].id,
            implementing_agency_id=agency_users[1].id,
            state_id=state_objects[9].id,
            district_id=district_objects["West Bengal_Kolkata"].id,
            description="Smart city development with IoT infrastructure",
            estimated_budget=Decimal("7500000000.00"),
            estimated_land_required_hectares=Decimal("300.000"),
            priority="medium",
            current_stage="compensation_assessment",
            status="under_review",
        ...
        for proj in [p2, p3, p4, p5]:
            ...
            for i in range(num_ms):
                ms = Milestone(
                    project_id=proj.id,
                    stage=STAGES[i],
                    title=STAGES[i].replace("_", " ").title(),
        ...
        for proj in projects:
            ...
            for p in range(num_parcels):
                ...
                dist = district_objects.get(
                    f"{proj.state.name}_{INDIAN_DISTRICTS.get(proj.state.name, [''])[0]}"
                )
                if not dist:
                ...
                parcel = LandParcel(
                    ...
                    land_type=random.choice(
                        ["agricultural", "residential", "commercial", "agricultural", "other"]
                    ),
        ...
        for i, parcel in enumerate(parcels[:60]):
            owner_name = (
                f"Land Owner Group {i // 5 + 1}"
                if i < 20
                else f"Family {i - 19}"
                if i < 40
                else f"Patel Family {i - 39}"
            )
            lo = LandOwner(
                parcel_id=parcel.id,
                full_name=owner_name,
                aadhaar_masked=f"{rand(1000, 9999):04d}****{rand(1000, 9999):04d}",
                phone=f"+91-98100-{rand(100, 999):03d}",
                email=f"owner{i}@email.com",
        ...
        for i, parcel in enumerate(parcels[:40]):
            sr = SurveyRecord(
                parcel_id=parcel.id,
                surveyed_by=field_users[i % len(field_users)].id,
        ...
        for i, parcel in enumerate(parcels[:30]):
            ...
            obj = Objection(
                ...
                hearing_date=random_date(date(2024, 1, 1), date(2024, 12, 31))
                if obj_status == "resolved"
                else None,
                status=obj_status,
                resolution_remarks=f"Objection resolved - compensation revised"
                if obj_status == "resolved"
                else None,
                resolved_by=district_users[i % len(district_users)].id
                if obj_status == "resolved"
                else None,
            )
            db.add(obj)
        ...
        for proj in [p1, p2, p5, p12, p14]:
            ...
            for f in range(num_families):
                rr = RehabilitationFamily(
                    project_id=proj.id,
                    family_head_name=f"Family Head {f + 1}",
                    ...
                    housing_benefit_status=random.choice(
                        ["not_started", "in_progress", "provided"]
                    ),
        ...
        for i, proj in enumerate(projects[:10]):
            for d in range(rand(3, 7)):
                ...
                doc = Document(
                    project_id=proj.id,
                    uploaded_by=agency_users[i % len(agency_users)].id,
                    doc_type=dt,
        ...
        audit_entries = [
            (
                "created",
                None,
                {"name": p1.name, "stage": "project_proposal"},
                "Project NH-44 Widening created",
            ),
            (
                "updated",
                {"stage": "project_proposal"},
                {"stage": "dpr_upload"},
                "DPR uploaded and submitted",
            ),
            (
                "updated",
                {"stage": "dpr_upload"},
                {"stage": "land_requirement"},
                "Land requirement assessment started",
            ),
            (
                "updated",
                {"stage": "land_requirement"},
                {"stage": "state_review"},
                "State review initiated",
            ),
            (
                "updated",
                {"stage": "state_review"},
            ...
            (
                "updated",
                {"stage": "gis_mapping"},
                {"stage": "legal_notification"},
                "Legal notification issued",
            ),
            (
                "updated",
                {"stage": "legal_notification"},
                {"stage": "objection_handling"},
                "Objection handling phase started",
            ),
            (
                "updated",
                {"stage": "objection_handling"},
                {"stage": "compensation_assessment"},
                "Compensation assessment completed",
            ),
            (
                "updated",
                {"stage": "compensation_assessment"},
                {"stage": "award_declaration"},
                "Award declaration approved",
            ),
            (
                "updated",
                {"stage": "award_declaration"},
                {"stage": "payment_disbursement"},
                "Payment disbursement initiated",
            ),
            (
                "updated",
                {"stage": "payment_disbursement"},
                {"stage": "physical_possession"},
        ...
        for action, old_val, new_val, remarks in audit_entries:
            al = AuditLog(
                ...
                new_value=new_val,
                remarks=remarks,
                ip_address=f"192.168.1.{rand(1, 254)}",
