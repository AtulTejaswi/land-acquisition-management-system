...
class Compensation(Base, TimestampMixin):
    ...
    status = Column(SAEnum(CompensationStatus, name="compensation_status_enum"), default=CompensationStatus.draft, nullable=False)
...
class Payment(Base, TimestampMixin):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    compensation_id = Column(UUID(as_uuid=True), ForeignKey("compensation.id"), nullable=False, index=True)
    land_owner_id = Column(UUID(as_uuid=True), ForeignKey("land_owners.id"), nullable=False, index=True)
    ...
    bank_verification_status = Column(
        ...
        default=BankVerificationStatus.pending, nullable=False
    )
