...
class LandParcel(Base, TimestampMixin, SoftDeleteMixin):
    ...
    geom = Column(Text, nullable=True)  # Will store GeoJSON geometry as text; PostGIS GEOMETRY handled at DB level
    land_type = Column(SAEnum(LandType, name="land_type_enum"), default=LandType.agricultural, nullable=False)
    ownership_status = Column(SAEnum(OwnershipStatus, name="ownership_status_enum"), default=OwnershipStatus.private, nullable=False)
    verification_status = Column(SAEnum(VerificationStatus, name="verification_status_enum"), default=VerificationStatus.pending, nullable=False)
...
class SurveyRecord(Base, TimestampMixin):
    ...
    status = Column(SAEnum(SurveyStatus, name="survey_status_enum"), default=SurveyStatus.scheduled, nullable=False)
