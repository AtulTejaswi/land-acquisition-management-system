...
from app.models.project import Ministry, ProjectCategory, Project, Milestone, ProjectStatus, ProjectPriority, MilestoneStatus, STAGES
    ...
    async with async_session() as db:
        ...
        states_data = [
            ...
            ("Tamil Nadu", "TN", "South", [
                ...
                ("Coimbatore", "CJB", ["Pollachi", "Mettupalayam", "Sulur", "Kinathukadavu", "Valparai"]),
            ]),
            ("Uttar Pradesh", "UP", "North", [
                ("Lucknow", "LKO", ["Lucknow", "Mohanlalganj", "Bakshi Ka Talaab", "Sarojini Nagar", "Mal"]),
                ("Varanasi", "VNS", ["Varanasi", "Ramnagar", "Pindra", "Arajiline", "Cholapur"]),
            ]),
            ("Gujarat", "GJ", "West", [
                ("Ahmedabad", "AMD", ["Ahmedabad City", "Daskroi", "Dholka", "Sanand", "Viramgam"]),
            ]),
            ("Karnataka", "KA", "South", [
                ("Bangalore", "BLR", ["Bangalore North", "Bangalore South", "Bangalore East", "Anekal", "Devanahalli"]),
            ]),
            ("Rajasthan", "RJ", "West", [
                ("Jaipur", "JAI", ["Jaipur", "Sanganer", "Amber", "Shahpura", "Chomu"]),
            ]),
            ("Andhra Pradesh", "AP", "South", [
                ("Visakhapatnam", "VSK", ["Visakhapatnam", "Gajuwaka", "Anakapalle", "Paderu", "Bheemunipatnam"]),
            ]),
            ("Telangana", "TS", "South", [
                ("Hyderabad", "HYD", ["Hyderabad", "Secunderabad", "Rangareddy", "Medchal", "Sangareddy"]),
            ]),
            ("Bihar", "BR", "East", [
        ...
        for s_name, s_code, s_region, districts in states_data:
            ...
            for d_name, d_code, villages in districts:
                ...
                for v_name in villages:
                    village = Village(district_id=district.id, tehsil=v_name, name=f"{v_name} Village", code=f"{d_code}-{v_name[:3].upper()}")
                    db.add(village)
                    all_villages.append(village)
        print(f"  ✅ {len(states)} states, {len(all_districts)} districts, {len(all_villages)} villages")
        ...
        # ===== CATEGORIES =====
        categories_data = ["Highway", "Railway", "Irrigation", "Industrial Corridor", "Renewable Energy", "Smart City", "Airport", "Defence", "Welfare"]
        categories = []
        ...
        user_specs = [
            ...
            # state_authority
            ("Anil Deshmukh", "anil@maharashtra.gov.in", "9876543212", "state_authority", states["MH"], None, None),
            ("Sunita Verma", "sunita@mp.gov.in", "9876543213", "state_authority", states["MP"], None, None),
            ("Karthik Iyer", "karthik@tn.gov.in", "9876543214", "state_authority", states["TN"], None, None),
            ("Amit Singh", "amit@up.gov.in", "9876543215", "state_authority", states["UP"], None, None),
            # district_officer
            ("Suresh Patil", "suresh@nagpur.gov.in", "9876543216", "district_officer", states["MH"], all_districts[0], None),
            ("Meena Yadav", "meena@bhopal.gov.in", "9876543217", "district_officer", states["MP"], all_districts[3], None),
            ("Vikram Reddy", "vikram@chennai.gov.in", "9876543218", "district_officer", states["TN"], all_districts[6], None),
            ("Deepak Joshi", "deepak@lucknow.gov.in", "9876543219", "district_officer", states["UP"], all_districts[9], None),
            ("Ravi Kumar", "ravi@indore.gov.in", "9876543220", "district_officer", states["MP"], all_districts[4], None),
            # agency
            ("NHAI Project Office", "agency@nhai.gov.in", "9876543221", "agency", states["MH"], None, "National Highways Authority of India"),
            ("IRCON International", "agency@ircon.co.in", "9876543222", "agency", states["MP"], None, "IRCON International Ltd"),
            ("L&T Infrastructure", "agency@ltinfra.com", "9876543223", "agency", states["TN"], None, "Larsen & Toubro Infrastructure"),
            ("NBCC India", "agency@nbcc.co.in", "9876543224", "agency", states["UP"], None, "National Buildings Construction Corp"),
            ("Adani Green", "agency@adanigreen.com", "9876543225", "agency", states["GJ"], None, "Adani Green Energy Ltd"),
            # field_officer
            ("Rahul Deshpande", "rahul.f@nlams.gov.in", "9876543226", "field_officer", states["MH"], all_districts[0], None),
            ("Sanjay Kulkarni", "sanjay.f@nlams.gov.in", "9876543227", "field_officer", states["MH"], all_districts[1], None),
            ("Prakash Tiwari", "prakash.f@nlams.gov.in", "9876543228", "field_officer", states["MP"], all_districts[3], None),
            ("Arun Nair", "arun.f@nlams.gov.in", "9876543229", "field_officer", states["TN"], all_districts[6], None),
            ("Manoj Singh", "manoj.f@nlams.gov.in", "9876543230", "field_officer", states["UP"], all_districts[9], None),
            ("Ajay Meena", "ajay.f@nlams.gov.in", "9876543231", "field_officer", states["MP"], all_districts[4], None),
            # citizens
            ("Ganesh Waghmare", "ganesh@email.com", "9876543232", "citizen", states["MH"], all_districts[0], None),
            ("Lata Bhosale", "lata@email.com", "9876543233", "citizen", states["MH"], all_districts[0], None),
            ("Ram Prasad", "ram@email.com", "9876543234", "citizen", states["MP"], all_districts[3], None),
            ("Sita Devi", "sita@email.com", "9876543235", "citizen", states["MP"], all_districts[3], None),
            ("Kumar Rajan", "kumar@email.com", "9876543236", "citizen", states["TN"], all_districts[6], None),
            ("Aarti Patel", "aarti@email.com", "9876543237", "citizen", states["GJ"], all_districts[12], None),
            ("Babu Lal", "babu@email.com", "9876543238", "citizen", states["UP"], all_districts[9], None),
            ("Champa Bai", "champa@email.com", "9876543239", "citizen", states["UP"], all_districts[9], None),
        ]
        ...
        projects_data = [
            ("NH-44 Widening — Nagpur to Betul", ministries[0], categories[0], users[11], states["MH"], all_districts[0], 2500000000, 120.5, "high", "active", "compensation_assessment"),
            ("Bhogapuram International Airport Land Pooling", ministries[3], categories[6], users[13], states["AP"], all_districts[15], 8000000000, 500.0, "critical", "active", "district_verification"),
            ("Delhi-Mumbai Industrial Corridor — Phase II", ministries[3], categories[3], users[14], states["MH"], all_districts[1], 15000000000, 2000.0, "critical", "approved", "gis_mapping"),
            ("Narmada River Irrigation Project — Betul", ministries[2], categories[2], users[12], states["MP"], all_districts[5], 1200000000, 800.0, "high", "active", "legal_notification"),
            ("Chennai Metro Phase III Extension", ministries[0], categories[5], users[13], states["TN"], all_districts[6], 5000000000, 150.0, "high", "under_review", "dpr_upload"),
            ("Lucknow–Varanasi Expressway", ministries[0], categories[0], users[14], states["UP"], all_districts[9], 3500000000, 350.0, "high", "active", "objection_handling"),
            ("Bhopal Smart City — Phase II", ministries[3], categories[5], users[12], states["MP"], all_districts[3], 2000000000, 50.0, "medium", "approved", "compensation_assessment"),
            ("Adani Solar Park — Rewa", ministries[4], categories[4], users[15], states["MP"], all_districts[3], 4000000000, 600.0, "medium", "active", "payment_disbursement"),
            ("Pune–Nagpur Railway Line Doubling", ministries[1], categories[1], users[12], states["MH"], all_districts[2], 6000000000, 400.0, "high", "delayed", "award_declaration"),
            ("Ahmedabad Metro Extension", ministries[0], categories[5], users[14], states["GJ"], all_districts[12], 3000000000, 80.0, "high", "active", "physical_possession"),
            ("Varanasi Heritage Corridor Project", ministries[3], categories[5], users[14], states["UP"], all_districts[10], 1500000000, 30.0, "critical", "active", "rehabilitation_resettlement"),
            ("Hyderabad Defence Corridor", ministries[3], categories[7], users[14], states["TS"], all_districts[14], 7000000000, 300.0, "critical", "approved", "state_review"),
            ("Jaipur Ring Road Project", ministries[0], categories[0], users[14], states["RJ"], all_districts[13], 2200000000, 180.0, "medium", "completed", "project_completion"),
            ("Coimbatore Water Supply Project", ministries[2], categories[2], users[13], states["TN"], all_districts[7], 800000000, 40.0, "medium", "draft", "project_proposal"),
            ("Patna–Gaya Railway Overhaul", ministries[1], categories[1], users[14], states["BR"], all_districts[15], 4500000000, 250.0, "high", "active", "compensation_assessment"),
        ]

        projects = []
        for i, (name, ministry, cat, agency_user, state, district, budget, land_ha, priority, status, stage) in enumerate(projects_data):
            p = Project(
                name=name, ministry_id=ministry.id, category_id=cat.id,
                ...
                estimated_budget=budget, estimated_land_required_hectares=land_ha,
                priority=priority, current_stage=stage, status=status,
                created_by=users[0].id,
                start_date=datetime.now(timezone.utc) - timedelta(days=random.randint(30, 365)),
                target_completion_date=datetime.now(timezone.utc) + timedelta(days=random.randint(180, 1095)),
            )
            db.add(p)
            projects.append(p)
        await db.flush()
        ...
        for proj in projects[:5]:
            ...
            for idx, stage in enumerate(STAGES):
                ...
                ms = Milestone(
                    ...
                    planned_date=proj.start_date + timedelta(days=30 * (idx + 1)) if proj.start_date else None,
                    actual_date=(proj.start_date + timedelta(days=30 * (idx + 1) + random.randint(-5, 15))) if is_completed and proj.start_date else None,
                    status=MilestoneStatus.completed if is_completed else (MilestoneStatus.in_progress if is_current else MilestoneStatus.pending),
                    responsible_officer_id=random.choice(users[:10]).id,
                    remarks="Completed successfully" if is_completed else ("In progress — monitoring" if is_current else None),
                )
                db.add(ms)
        await db.flush()
        ...
        # ===== LAND OWNERS =====
        owner_names = [
            ("Ganesh Waghmare", "9876543232"), ("Lata Bhosale", "9876543233"),
            ("Ram Prasad", "9876543234"), ("Sita Devi", "9876543235"),
            ("Kumar Rajan", "9876543236"), ("Aarti Patel", "9876543237"),
            ("Babu Lal", "9876543238"), ("Champa Bai", "9876543239"),
            ("Dattatraya Mahajan", "9876543240"), ("Savitribai Phule", "9876543241"),
            ("Tukaram Bholu", "9876543242"), ("Narayanrao Raut", "9876543243"),
        ]
        owners = []
        ...
        for proj in projects[:5]:
            ...
            for parcel in proj_parcels[:3]:
                ...
                comp = Compensation(
                    ...
                    assessment_date=datetime.now(timezone.utc) - timedelta(days=random.randint(10, 60)),
        ...
        for comp in compensations:
            ...
            for owner in parcel_owners[:1]:
                payment = Payment(
                    ...
                    amount=float(comp.total_award or 0) * (float(owner.share_percentage or 100) / 100),
                    ...
                    payment_status=random.choice(["pending", "processing", "disbursed", "disbursed"]),
                    disbursed_date=datetime.now(timezone.utc) - timedelta(days=random.randint(0, 30)) if random.random() > 0.5 else None,
                )
                db.add(payment)
        ...
        for proj in projects[:5]:
            for k in range(random.randint(3, 8)):
                fam = RehabilitationFamily(
                    ...
                    housing_benefit_status=random.choice(["provided", "in_progress", "not_started"]),
                    employment_benefit_status=random.choice(["provided", "in_progress", "not_started"]),
        ...
        for state_code, state_obj in states.items():
            for district in [d for d in all_districts if d.state_id == state_obj.id]:
                for lt in ["agricultural", "residential", "commercial"]:
                    base_rates = {"agricultural": 500000, "residential": 2000000, "commercial": 5000000}
                    cr = CircleRate(
